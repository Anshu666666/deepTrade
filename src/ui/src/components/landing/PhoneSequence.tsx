...
const PhoneSequence: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const renderFrame = (index: number) => {
      ...
      if (isMobile) {
        // Position phone cleanly in lower viewport below text and commands card with generous vertical margin
        const textBlockBottom = 365; // bottom of text + system commands card + margin when top-[125px] is applied
        const availableHeight = Math.max(logicalHeight - textBlockBottom - 20, 200);
        const baseScale = Math.min((availableHeight * 0.86) / img.height, (logicalWidth * 0.92) / (img.width * 0.42));
  ...
  return (
    <div ref={containerRef} className="w-full h-[100dvh] bg-black relative flex items-center justify-center overflow-hidden z-20">
      ...
      {/* Content block: centered comfortably below header with top-[125px] on mobile */}
      <div className="absolute left-4 right-4 sm:left-8 sm:right-auto md:left-32 lg:left-40 xl:left-[15%] max-w-xl z-10 text-white pointer-events-auto top-[125px] sm:top-[135px] md:top-0 bottom-auto md:bottom-0 box-border flex flex-col justify-start md:justify-center">
