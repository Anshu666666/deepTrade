...
export function TypingKeyboard({
  className,
  autoTypeText = "import { DeepTrade } from 'agents';\n\nagent.analyze('AAPL');",
  typingSpeed = [40, 120],
  scale = 0.8,
  accentColor = "#3b82f6",
  ...
  useEffect(() => {
    ...
    const typeNext = () => {
      ...
      const kc = char === " " ? 32 : char === "\n" ? 13 : char.toUpperCase().charCodeAt(0);

      if (KC_MAP[kc] !== undefined) pressKey(kc);
  ...
  return (
    <div className={cn("tk-container", className)} {...props}>
      <style>{`
        ...
        .tk-keyboard__front { width: 500px; height: 25px; transform: rotateX(-90deg) translateZ(80px); background-color: #1a1a1a; }
        .tk-keyboard__back  { width: 500px; height: 25px; transform: rotateX(90deg) translateZ(80px); background-color: #333; }
        .tk-keyboard__top   {
          display: flex; flex-direction: column; justify-content: space-around;
          width: 500px; height: 160px;
          transform: rotateY(0deg) translateZ(12.5px);
          background-image: linear-gradient(to bottom, color-mix(in srgb, ${accentColor} 40%, #000), color-mix(in srgb, #000 80%, #000));
        }
        .tk-keyboard__bottom {
          width: 500px; height: 160px;
        ...
        .tk-keyboard__right { width: 25px; height: 160px; transform: rotateY(90deg) translateZ(250px); background-color: #333; }
        .tk-keyboard__left  { width: 25px; height: 160px; transform: rotateY(90deg) translateZ(-250px); background-color: #111; }
        ...
        .tk-key__front { width: 30px; height: 8px; transform: rotateX(-90deg) translateZ(13.5px); background-color: #0f0f0f; }
        .tk-key__front--w2 { width: 60px; }
        .tk-key__front--w3 { width: 90px; }
        .tk-key__front--w6 { width: 195px; }

        .tk-key__back { width: 30px; height: 8px; transform: rotateX(90deg) translateZ(13.5px); background-color: #3a3a3a; }
        ...
        .tk-key__top {
          ...
          background-image: linear-gradient(to bottom, color-mix(in srgb, ${accentColor} 10%, #222), #1f1f1f);
          box-shadow: inset 0 0 0 0.5px rgba(255,255,255,0.1), 0 -1px 2px rgba(0,0,0,0.5);
        }
        .tk-key__top--w2 { width: 60px; }
        .tk-key__top--w3 { width: 90px; }
        .tk-key__top--w6 { width: 195px; }

        .tk-key__bottom { width: 30px; height: 27px; transform: rotateY(180deg) translateZ(4px); background-color: #0f0f0f; }
        .tk-key__bottom--w2 { width: 60px; }
        .tk-key__bottom--w3 { width: 90px; }
        .tk-key__bottom--w6 { width: 195px; }

        .tk-key__right { width: 8px; height: 27px; transform: rotateY(90deg) translateZ(15px); background-color: #0f0f0f; }
        ...
        .tk-key--down > .tk-key__top {
          background: color-mix(in srgb, ${secondaryAccent} 40%, #555) !important;
        }
      `}</style>

      <div className="tk-main tk-flex" ref={mainRef}>
        <div className="tk-keyboard tk-flex" ref={kbRef}>
          <div className="tk-screen tk-flex text-[10px]" ref={screenRef} style={{fontFamily: 'monospace'}} />
