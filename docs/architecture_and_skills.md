...
flowchart TB
    ...
    subgraph AgentLayer["3. DeepAgent Execution Engine (src/agent/graph.py)"]
        SUPERVISOR["🧠 DeepTrade Agent (Supervisor)<br/>(create_deep_agent)"]
        SKILLS["📁 Progressive Skills System<br/>(/skills/upstox/SKILL.md)"]
        SUPERVISOR <--> SKILLS
    end

    subgraph ToolsLayer["4. Unified Tool Suite (src/agent/)"]
        TOOL_EXA["🌐 search_web (Exa Search)"]
        TOOL_VALYU["📑 search_financial_data (Valyu SEC & Fundamentals)"]
        TOOL_UPSTOX_MKT["📈 upstox_get_market_data (LTP / OHLC / Historical)"]
        TOOL_UPSTOX_ORD["⚡ upstox_place_order / modify / cancel"]
        TOOL_UPSTOX_PORT["💼 upstox_get_holdings / positions / funds / order_book"]
        TOOL_FILES["📝 write_file (LangGraph Postgres Store / Filesystem)"]
    end
    ...
    PRECHECK --> SUPERVISOR
    SUPERVISOR --> TOOL_EXA
    SUPERVISOR --> TOOL_VALYU
    SUPERVISOR --> TOOL_UPSTOX_MKT
    SUPERVISOR --> TOOL_UPSTOX_ORD
    SUPERVISOR --> TOOL_UPSTOX_PORT
    SUPERVISOR --> TOOL_FILES
    TOOL_FILES --> POSTGRES
    SUPERVISOR -.->|Thread State Checkpoint| POSTGRES
    TOOL_UPSTOX_ORD -->|Live Mode| BROKER
    TOOL_UPSTOX_ORD -->|Sandbox Mode| SANDBOX_LEDGER
    TOOL_UPSTOX_PORT -->|Live Mode| BROKER
    TOOL_UPSTOX_PORT -->|Sandbox Mode| SANDBOX_LEDGER
...
- **High-Level Purpose:** An autonomous financial research and analysis agent that gathers real-time market data, web intelligence, and SEC filings to synthesize structured research reports and execute live trades interactively via both a Web UI and a Telegram Bot.
...
- **Key Constraints/Design Philosophy:** Event-driven and highly observable. Relies on a single unified DeepAgent (`Supervisor`) equipped with comprehensive tools and progressive skills (`/skills/upstox/`). Relies heavily on Server-Sent Events (SSE) streaming for the Web UI and background task webhooks for Telegram to surface granular agent interactions (reasoning tokens, tool calls, and artifact emissions). State is checkpointed to Postgres implicitly via LangGraph abstractions.
...
- `src/agent/graph.py`: Constructs the `deep_agent` graph using `create_deep_agent`, wiring the Supervisor with all external tools, `/skills/` directory, and injecting a `CompositeBackend` to route artifact storage to Postgres.
- `src/agent/tools.py`: Implements external search integrations (`search_financial_data` using Valyu and `search_web` using Exa).
- `src/agent/upstox_tools.py`: Implements the Upstox API tool wrappers (fetching market quotes, orders, holdings, positions, funds, and placing interactive orders with Telegram confirmations).
...
- `src/agent/config_loader.py`: Dynamically loads Markdown-based system prompts from `src/agent/config/prompts/` to feed the agent.
- `skills/upstox/SKILL.md`: Progressive skill definition providing guidelines, tool usage constraints, and confirmation rules for Upstox trading.
...
sequenceDiagram
    ...
    participant Agent as 🧠 DeepTrade Agent (Supervisor)
    ...
    alt Token Expired or IP Changed
        ...
        Server->>Agent: astream(state, config)
        loop Agent Execution & Tool Invocation
            Agent->>Broker: Tool Calls (LTP, Filings, Web Search, Orders)
            Broker-->>Agent: Return Tool Results
            Agent->>DB: Save Checkpoints & Query Events
            Agent-->>Server: Stream Reasoning & Status Tokens
        end
        Agent-->>User: Deliver Final Response & Markdown Artifacts
    end
```
...
2. **Pre-Flight Session Check:** `check_and_sync_live_session()` validates token and IP before invoking the agent.
...
4. **Event Interception & Logging:** As the Supervisor emits updates, `run_graph` parses them into distinct `log_type`s (e.g., `REASONING`, `TOOL_CALL`, `ARTIFACT`).
...
6. **External Tooling:** If a tool call occurs, `src/agent/tools.py` hits Valyu or Exa, returning the raw text string back into the agent loop.
...
sequenceDiagram
    ...
    Note over Tool: Agent execution suspended for up to 5 minutes
...
- `agent` (TEXT): Which agent generated the event (e.g., `Supervisor`).
- `content` (TEXT): Stringified event payload.
- `event_metadata` (JSONB): Parsed JSON metadata (like tool args).
...
stateDiagram-v2
    ...
    REASONING --> TOOL_CALL: Agent Calls Tool
...
- **TOOL_CALL**: Agent requests to use a tool (`{"type": "log", "log_type": "TOOL_CALL", "agent": "Supervisor", "tool": "search_web", "content": "{\"query\": \"AAPL\"}"}`).
- **TOOL_RESULT**: External API response payload (`{"type": "log", "log_type": "TOOL_RESULT", "agent": "Supervisor", "tool": "search_web", "content": "Raw search results..."}`).
- **ARTIFACT**: A saved markdown document generated during research (`{"type": "log", "log_type": "ARTIFACT", "agent": "Supervisor", "path": "/research/report.md", "content": "Markdown body..."}`).
